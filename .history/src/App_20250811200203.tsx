  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        authorityWalletInfo = new PrivyWallet(walletAdapter, new PublicKey(user.wallet.address), true);
      } else {
        ...
        if (user.wallet.walletClientType === SupportedWalletType.PHANTOM_ADAPTER) {
            ...
            // 使用自定义适配器创建PrivyWallet实例
            authorityWalletInfo = new PrivyWallet(walletAdapter, new PublicKey(user.wallet.address), false);
        }
      }
