...
// 导入SolConSdk及其相关类型
import SolConSdk, { getEnvParams, SupportedWalletType, PrivyWallet, PhantomWallet } from 'solconsdk';
...
function LoginButton() {
  ...
  // 使用 useSolanaWallets 获取钱包地址
  const primaryWallet = solanaWallets && solanaWallets.length > 0 ? solanaWallets[0] : null;
...
function CreateMarketTest({ user, solanaWallets }: { user: any; solanaWallets: any[] }) {
  ...
  return (
    <PrivyProvider
      ...
      config={{
        ...
        embeddedWallets: {
          createOnLogin: 'all-users' // 为所有用户创建嵌入式钱包
        },
