...
// 根据solconsdk的WalletInfo接口定义
export interface WalletInfo {
  publicKey: PublicKey;
  type?: string;
  partialSign: (transaction: Transaction) => Promise<Transaction>;
  signAndSendTransaction: (transaction: Transaction) => Promise<string>;
  setConnection: (connection: Connection) => void;
  signMessage: (message: string) => Promise<string>;
  partialSignV0: (transaction: VersionedTransaction) => Promise<VersionedTransaction>;
  signAndSendTransactionV0: (transaction: VersionedTransaction) => Promise<string>;
  broadcastTransactions: (signedTransactions: Transaction[], broadcastUrl: string) => Promise<string[]>;
  checkSolBalance: (minBalance?: number) => Promise<void>;
}
