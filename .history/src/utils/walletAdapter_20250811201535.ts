...
/**
 * 自定义的Privy钱包适配器，用于实现solconsdk的WalletInfo接口
 */
export class PrivyWalletAdapter implements WalletInfo {
  ...
  constructor(
    ...
    this.type = isGasless ? "privy-gasless" : "privy";
    // 检查是否为Phantom钱包
  ...
  setConnection(connection: Connection): void {
    ...
    console.log("设置连接:", connection);
  }

  /**
   * 部分签名交易 (实现WalletInfo接口的partialSign方法)
   */
  async partialSign(transaction: Transaction): Promise<Transaction> {
    console.log("PrivyWalletAdapter.partialSign 被调用");
    console.log("钱包对象:", this.wallet);
    console.log("钱包类型:", this.type);
    console.log("是否为Phantom钱包:", this.isPhantom);
    ...
    throw new Error(
      "钱包不支持任何已知的签名方法，请使用支持signTransaction的钱包"
    );
  }
   ...
   */
  private async tryPhantomSpecificSigning(
      ...
      if (typeof this.wallet[method] === "function") {
        ...
        try {
          ...
          if (method === "signAndSendTransaction") {
            ...
            const result = await this.wallet[method](transaction, {
              skipPreflight: true,
            });
            ...
            throw new Error(
              "无法从signAndSendTransaction结果中提取签名后的交易"
            );
          }
  ...
  async signAllTransactions(
    ...
    console.error("钱包不支持任何已知的签名方法");
    throw new Error(
      "钱包不支持任何已知的签名方法，请使用支持signTransaction或signAllTransactions的钱包"
    );
  }
