  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        // 使用SDK提供的PrivyWallet
        authorityWalletInfo = new PrivyWallet(user.wallet, new PublicKey(user.wallet.address), true);
        ...
        if (user.wallet.walletClientType === SupportedWalletType.PHANTOM_ADAPTER) {
            ...
            // 使用SDK提供的PhantomWallet
            authorityWalletInfo = new PhantomWallet(new PublicKey(user.wallet.address));
            console.log('创建的Phantom钱包适配器:', authorityWalletInfo);
        } else {
            // 其他外部钱包使用PrivyWallet
            authorityWalletInfo = new PrivyWallet(user.wallet, new PublicKey(user.wallet.address), false);
