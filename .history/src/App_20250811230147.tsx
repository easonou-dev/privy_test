...
function LoginButton() {
  ...
  return (
    <div>
      ...
      {walletAddress ? (
        <div>
          <p>当前主钱包地址: {walletAddress}</p>
          <p>钱包类型: {primaryWallet?.connectorType === 'embedded' ? '嵌入式钱包 (Privy)' : '外部钱包 (如 Phantom)'}</p>
          {solanaWallets && solanaWallets.length > 1 && (
            <div style={{ marginTop: '10px', padding: '10px', backgroundColor: '#f5f5f5', borderRadius: '5px' }}>
              <p><strong>可用钱包列表:</strong></p>
              {solanaWallets.map((wallet, index) => (
                <div key={wallet.address} style={{ marginBottom: '5px' }}>
                  <span style={{ fontFamily: 'monospace', fontSize: '12px' }}>
                    {index + 1}. {wallet.address.slice(0, 8)}...{wallet.address.slice(-8)} 
                    ({wallet.connectorType === 'embedded' ? '嵌入式' : '外部'})
                    {wallet.address === primaryWallet?.address && ' ← 当前主钱包'}
                  </span>
                </div>
              ))}
            </div>
