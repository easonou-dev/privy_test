...
import { PrivyWalletAdapter, WalletInfo } from './utils/walletAdapter';


// 导入SolConSdk及其相关类型
// 注意：我们使用require语法来避免TypeScript的类型检查问题
const SolConSdk = require('solconsdk').default;
const { getEnvParams, SupportedWalletType } = require('solconsdk');

// 由于我们无法直接访问solconsdk的类型定义，我们需要自己定义一些接口
...
interface PhantomWallet {
  // 添加必要的属性和方法
  new (publicKey: PublicKey): any;
}

interface PrivyWallet {
  // 添加必要的属性和方法
  new (wallet: any, publicKey: PublicKey, isGasless: boolean): any;
}

// 获取SolConSdk中的类
const ServerWalletClass = require('solconsdk').ServerWallet;
const PhantomWalletClass = require('solconsdk').PhantomWallet;
const PrivyWalletClass = require('solconsdk').PrivyWallet;
...
function CreateMarketTest({ user }: { user: any }) {
  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      let authorityWalletInfo: WalletInfo; // 使用WalletInfo接口类型
      // 使用自定义的PrivyWalletAdapter来处理钱包操作
      if (isGaslessTest) {
        ...
        // 直接使用PrivyWalletAdapter作为authorityWalletInfo
        authorityWalletInfo = new PrivyWalletAdapter(user.wallet, new PublicKey(user.wallet.address), true);
        ...
        // 对于 Phantom 钱包，使用SDK提供的PhantomWallet类
        if (user.wallet.walletClientType === SupportedWalletType.PHANTOM_ADAPTER) {
            // 检查Phantom钱包对象是否有必要的方法
            console.log('Phantom钱包对象:', user.wallet);
            console.log('Phantom钱包方法:', Object.getOwnPropertyNames(user.wallet));
            
            // 创建一个特殊的适配器用于Phantom钱包
            authorityWalletInfo = new PrivyWalletAdapter(user.wallet, new PublicKey(user.wallet.address), false);
            console.log('创建的Phantom钱包适配器:', authorityWalletInfo);
        } else {
            // 其他外部钱包也使用PrivyWalletAdapter
            authorityWalletInfo = new PrivyWalletAdapter(user.wallet, new PublicKey(user.wallet.address), false);
  ...
  return (
    <PrivyProvider
      ...
      config={{
        ...
        solanaClusters: [{name: 'mainnet-beta', rpcUrl: 'https://mainnet.helius-rpc.com/?api-key=7b73474a-082e-4823-be84-8dd53275cb7a'},{name: 'devnet', rpcUrl: 'https://devnet.helius-rpc.com/?api-key=7b73474a-082e-4823-be84-8dd53275cb7a'}],
        externalWallets: {
          solana: {
