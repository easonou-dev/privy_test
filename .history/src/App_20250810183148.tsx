...
function LoginButton() {
  ...
  // ConnectedWallet 类型有 `address` 和 `chainType`。
  if (user.wallet && user.wallet.chainType === 'solana') {
    walletAddress = user.wallet.address;
  } else {
    // 如果 user.wallet 不是 Solana 类型，或者未设置（例如，对于一些旧版嵌入式钱包），
...
// 如果这里仍然报错“找不到模块”，请检查 solConSdk 项目的构建和 tsconfig.json
...
function CreateTopicButtons() {
  ...
  const handleCreateTopicWithGasless = async () => {
    ...
    // 获取 Privy 嵌入式钱包的实际 Solana 地址
    let privyEmbeddedWalletAddress: PublicKey | undefined;
    
    // 优先从 user.wallet 获取，如果它是 Privy 嵌入式钱包
    // user.wallet.walletClientType === 'privy' 表示是 Privy 托管的嵌入式钱包
    if (user.wallet && user.wallet.walletClientType === 'privy' && user.wallet.chainType === 'solana') {
    ...
    // 这个检查更严格，确保只有在连接 Privy 嵌入式钱包时才触发 Gas 代付
  ...
  const handleCreateTopicWithPhantom = async () => {
    ...
    // user.wallet.solana 只有在连接了 Solana 外部钱包时才存在 (ConnectedSolanaWallet 类型)
