  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        // 创建自定义钱包适配器
        const walletAdapter = new PrivyWalletAdapter(user.wallet, new PublicKey(user.wallet.address), true);
        console.log('创建的钱包适配器:', walletAdapter);
        
        // 使用自定义适配器创建PrivyWallet实例
        authorityWalletInfo = new PrivyWalletClass(walletAdapter, new PublicKey(user.wallet.address), true);
      } else {
        ...
        if (user.wallet.walletClientType === SupportedWalletType.PHANTOM_ADAPTER) {
            authorityWalletInfo = new PhantomWalletClass(new PublicKey(user.wallet.address));
        } else {
            // 假设其他 Privy 连接的外部钱包也是 non-gasless
            // 创建自定义钱包适配器
            const walletAdapter = new PrivyWalletAdapter(user.wallet, new PublicKey(user.wallet.address), false);
            console.log('创建的钱包适配器:', walletAdapter);
            
            // 使用自定义适配器创建PrivyWallet实例
            authorityWalletInfo = new PrivyWalletClass(walletAdapter, new PublicKey(user.wallet.address), false);
        }
      }
