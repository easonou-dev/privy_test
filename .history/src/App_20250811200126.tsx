  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        // 确保user.wallet有signAllTransactions方法
        if (!user.wallet.signAllTransactions) {
            console.error('错误: user.wallet没有signAllTransactions方法');
            console.log('user.wallet对象:', user.wallet);
            // 尝试查找signAllTransactions方法
            const methods = Object.getOwnPropertyNames(user.wallet);
            console.log('user.wallet可用方法:', methods);
            setMessage('错误: 钱包不支持signAllTransactions方法，请检查控制台获取更多信息。');
            setLoading(false);
            return;
        }
