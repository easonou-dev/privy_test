...
function LoginButton() {
  ...
  // 根据最近的登录方法选择主钱包
  // 检查用户最近的登录方法来决定主钱包
  const lastLoginMethod = user?.linkedAccounts?.[user.linkedAccounts.length - 1]?.type;
  console.log('Last login method:', lastLoginMethod);
  console.log('User linked accounts:', user?.linkedAccounts);
  
  let primaryWallet = null;
  
  // 如果最后登录方法是社交登录（google等），优先选择嵌入式钱包
  if (lastLoginMethod === 'google_oauth' || lastLoginMethod === 'twitter_oauth' || lastLoginMethod === 'discord_oauth') {
    primaryWallet = solanaWallets?.find(wallet => wallet.connectorType === 'embedded');
    console.log('Social login detected, selecting embedded wallet:', primaryWallet);
  }
  
  // 如果没有找到嵌入式钱包，或者最后登录方法是钱包连接，选择外部钱包
  if (!primaryWallet) {
    primaryWallet = solanaWallets?.find(wallet => wallet.connectorType !== 'embedded') || 
                   (solanaWallets && solanaWallets.length > 0 ? solanaWallets[0] : null);
    console.log('Selecting external wallet or fallback:', primaryWallet);
  }
  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        if (primaryWallet.connectorType !== 'embedded') {
        ...
        // 使用SDK提供的PrivyWallet
        authorityWalletInfo = new PrivyWallet(primaryWallet, new PublicKey(primaryWallet.address), true);
        console.log('创建的钱包适配器 (Gasless):', authorityWalletInfo);
      } else {
        // Non-Gasless 模式 (外部钱包)
        if (primaryWallet.connectorType === 'embedded') {
            setMessage('错误: 请连接外部钱包 (如 Phantom) 测试 Non-Gasless 模式。');
            setLoading(false);
            return;
        }
        
        // 对于外部钱包，检查钱包类型
        console.log('外部钱包对象:', primaryWallet);
        console.log('钱包连接器类型:', primaryWallet.connectorType);
        
        // 使用SDK提供的PrivyWallet处理外部钱包
        authorityWalletInfo = new PrivyWallet(primaryWallet, new PublicKey(primaryWallet.address), false);
