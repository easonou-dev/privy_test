...
// 修正 solconsdk 模块的导入路径，使用包名而不是相对路径
import SolConSdk, {
  ENV_PARAMS,
  getEnvParams,
  QuestionOption,
...
import {
  ...
  ServerWallet,
  SupportedWalletType,
} from 'solconsdk/src/common/walletInfo';
...
function CreateMarketTest({ user }: { user: any }) {
  ...
  const commonEnv = getEnvParams();
  // 实例化一个 ServerWallet 作为平台代付的 feePayer
  const serverFeePayer = new ServerWallet(
    commonEnv.FEE_PAYER.secretKey,
    commonEnv.BROADCAST_URL
  );
  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      if (isGaslessTest) {
        ...
        authorityWalletInfo = new PrivyWallet(user.wallet, true); // isGasless: true
      } else {
        ...
        if (user.wallet.walletClientType === SupportedWalletType.PHANTOM_ADAPTER) {
            authorityWalletInfo = new PhantomWallet(user.wallet.address, user.wallet.solana);
        } else {
            // 假设其他 Privy 连接的外部钱包也是 non-gasless
            authorityWalletInfo = new PrivyWallet(user.wallet, false); // isGasless: false
        }
      }

      const solConSdk = new SolConSdk(
        serverFeePayer, // Platform fee payer
        commonEnv.RPC_URL,
        commonEnv.WS_URL,
        commonEnv.COLLATERAL_MINT,
        commonEnv.BROADCAST_URL
      );

      // 设置 SDK 的 feePayer，以便 createMarketAndQuestionsAuto 内部使用
      solConSdk.setFeePayer(serverFeePayer);
      solConSdk.setNetwork(commonEnv.NETWORK_NAME);

      // 如果有全局查找表，设置它
      if (commonEnv.GLOBAL_LOOKUP_TABLE) {
          solConSdk.setGlobalLookupTable(new PublicKey(commonEnv.GLOBAL_LOOKUP_TABLE));
      }


      console.log(`正在使用 ${isGaslessTest ? 'Gasless' : 'Non-Gasless'} 模式创建市场...`);
      console.log('Authority WalletInfo type:', authorityWalletInfo.type);
      if (authorityWalletInfo instanceof PrivyWallet) {
        console.log('PrivyWallet isGasless:', (authorityWalletInfo as PrivyWallet).isGasless);
      }
      console.log('Market Data:', createMarketData);

      const result = await solConSdk.createMarketAndQuestionsAuto(
        authorityWalletInfo,
