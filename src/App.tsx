...
function LoginButton() {
  ...
  // 检查用户是否有社交登录账户（谷歌等）
  const hasSocialAccount = user?.linkedAccounts?.some(account => 
    account.type === 'google_oauth' || 
    account.type === 'twitter_oauth' || 
    account.type === 'discord_oauth'
  );
  
  console.log('Has social account:', hasSocialAccount);
  
  let primaryWallet = null;
  
  // 如果用户有社交登录账户，优先选择嵌入式钱包
  if (hasSocialAccount) {
    primaryWallet = solanaWallets?.find(wallet => wallet.connectorType === 'embedded');
    console.log('Social account detected, selecting embedded wallet:', primaryWallet);
  }
  
  // 如果没有找到嵌入式钱包，或者用户没有社交账户，选择外部钱包
  if (!primaryWallet) {
    primaryWallet = solanaWallets?.find(wallet => wallet.connectorType !== 'embedded') || 
...
function CreateMarketTest({ solanaWallets, primaryWallet }: { user: any; solanaWallets: any[]; primaryWallet: any }) {
  ...
  const handleCreateMarket = async (isGaslessTest: boolean) => {
    ...
    try {
      ...
      // 使用传入的主钱包，而不是重新选择
      // const primaryWallet = solanaWallets[0]; // 移除这行，使用传入的primaryWallet
